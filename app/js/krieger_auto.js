...
function kriegerAutoWinChance(dd, equipped, tier) {
...
function kriegerAutoBestLoadout(dd, tier, currentEquipped) {
  ...
  for (const w of ws) for (const a of as) for (const t of ts) {
    ...
    const est = kriegerAutoWinChance(dd, eq, tier);
...
async function kriegerAutoRun(member, state, seed, COLS, ROWS, MARGIN) {
  ...
  const loadoutCache = {};                               // Tier → bestes Loadout (1× pro Lauf)
  ...
  try {
    ...
    while (rep.steps < budget && actions < KRIEGER_AUTO_MAX_ACTIONS) {
      ...
      if (encounter && encounter.tier !== 'boss') {
        ...
        if (!loadoutCache[tier]) loadoutCache[tier] = kriegerAutoBestLoadout(state.dd, tier, state.dd.equipped);
        const best = loadoutCache[tier];
        ...
        let result;
        try { result = await DB.dungeonFight(member.id, tier); }
        catch (e) { rep.endReason = 'error'; break; }
